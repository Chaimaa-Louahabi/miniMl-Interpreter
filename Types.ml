...
let rec type_of_expr expr env =
  ...
  and
    ...
    ruleFunction par body env =
        ...
        let tres = (type_of_expr body ((par,tpar)::env)) in
        FunctionType (tpar,tres)

  and 
    (* ..................................................................*)
    ruleCall fct par env = 
        let tfun = (type_of_expr fct env) in
        let tpar_eff = (type_of_expr par env) in
        let ut, ur = unify tfun (FunctionType (tpar_eff,  newVariable ())) in
        if ur then 
            match ut with
            |FunctionType (_,  result_type) -> result_type
            |_ -> ErrorType
        else ErrorType 
  ...
  and
    (* .................................................................*)
    ruleRef expr env = 
        let t_expr = (type_of_expr expr env) in
        match t_expr  with 
        |ErrorType -> ErrorType
  ...
  and
    (* ..................................................................*)
    ruleRead expr env = 
        match (type_of_expr expr env) with
  ...
  and
    ...
    ruleWrite left right env = 
        ...
        |ReferenceType t_left -> 
            let t_right = (type_of_expr right env) in
            let ut,ur = unify t_right t_left in
  ...
  and
    ...
    ruleWhile cond body env = 
        ...
        |BooleanType -> 
            (match (type_of_expr body env) with
            |UnitType -> UnitType
            |_ -> ErrorType)
        | _ -> ErrorType
