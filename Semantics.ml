...
ruleRead expr mem env = 
    ...
    |((ReferenceValue adr), mem2) -> 
            (match (lookforMem adr mem2) with
            |Found value -> (value, mem2)
            |_ ->(ErrorValue (UnknownReferenceError adr), mem))
...
ruleWrite refexpr valexpr mem env = 
    let (eval, mem2)= value_of_expr (valexpr, mem) env in 
         match eval with
         |(ErrorValue _) as result -> (result,  mem)
         |_-> (match (value_of_expr (refexpr, mem2) env) with
              |(ReferenceValue adr, mem3) -> 
                      (match lookforMem adr mem with
                      | Found _ -> (NullValue, (adr, eval)::mem3)
                      | _ ->(ErrorValue (UnknownReferenceError adr), mem))
              |_-> ((ErrorValue TypeMismatchError),mem))
...
ruleSequence left right mem env =
    let (valueleft, mem2) = value_of_expr (left, mem) env in
    match valueleft with
    |(ErrorValue _) as result -> (result,  mem)
    |NullValue-> (let (valueright, mem3) = value_of_expr (right, mem2) env in 
            match valueright with
            |(ErrorValue _) as result -> (result, mem)
            |_ -> (valueright, mem3))
    | _ -> ((ErrorValue TypeMismatchError),mem)
...
ruleWhile cond body mem env = let (cond_v, mem2) = value_of_expr (cond, mem) env in
    match cond_v with
    | BooleanValue value -> (
        if (value) then
            let (body_v, mem3) = value_of_expr (body, mem2) env in
            match body_v with
            |NullValue-> ruleWhile cond body mem3 env
...
ruleReference expr mem env =
    let (eval, mem2)= value_of_expr (expr, mem) env in
          match eval with
          |(ErrorValue _) as result -> (result,  mem)
          |_-> let adr = newReference () in ((ReferenceValue adr), (adr, eval)::mem2)
